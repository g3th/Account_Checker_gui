...
class split_combos():
	...
	def __init__(self, root):
		...
		self.split_text_completed= Label(root, bg='goldenrod3',text='Done Splitting - Total Combos {}'.format(str(len(clean_list))), font=('Arial',18))
	
	def destroy_split_button(self):
		self.split_combos_button.destroy()
	
	def destroy_info_label(self):
		self.split_text_complete.destroy()
